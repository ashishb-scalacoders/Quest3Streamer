...
namespace FMSolution.FMWebSocket
{
    ...
    public class FMWebSocketManager_Editor : UnityEditor.Editor
    {
        ...
        Vector2 scrollPosition_NetworkViews;
        public FMWebSocketView source;
        ...
        public override void OnInspectorGUI()
        {
            ...
            GUILayout.BeginVertical("box");
            {
                if (!FMWebSocket.EditorShowNetworkViews || !drawSuccess)
                {
                    GUILayout.BeginHorizontal();
                    GUI.skin.button.alignment = TextAnchor.MiddleLeft;
                    if (GUILayout.Button("+ Networked Views")) FMWebSocket.EditorShowNetworkViews = true;
                    GUI.skin.button.alignment = TextAnchor.MiddleCenter;
                    GUILayout.EndHorizontal();
                }
                else
                {
                    GUILayout.BeginHorizontal();
                    GUI.skin.button.alignment = TextAnchor.MiddleLeft;
                    if (GUILayout.Button("- Network Views")) FMWebSocket.EditorShowNetworkViews = false;
                    GUI.skin.button.alignment = TextAnchor.MiddleCenter;
                    GUILayout.EndHorizontal();
                    ...
                    {
                        GUILayout.BeginVertical("box");
                        if (!Application.isPlaying && !Application.IsPlaying(this))
                        {
                            GUI.backgroundColor = Color.green;
                            if (GUILayout.Button("Update Network Views")) FMWebSocket.UpdateRoomViewIDsAll();
                            GUI.backgroundColor = Color.white;
                        }
                        GUILayout.EndVertical();

                        int[] _networkedViewsKey = FMWebSocket.NetworkViews.Keys.ToArray();
                        GUILayout.BeginVertical("box");
                        {
                            EditorGUILayout.BeginHorizontal();
                            GUILayout.Label($@"Found in scene: {_networkedViewsKey.Length}");
                            GUILayout.EndHorizontal();
                        }
                        GUILayout.EndVertical();
                        ...
                        GUILayout.BeginVertical("box");
                        {
                            float _scrollViewHeight = Mathf.Clamp(_networkedViewsKey.Length * 20, 20, 150);
                            scrollPosition_NetworkViews = EditorGUILayout.BeginScrollView(scrollPosition_NetworkViews, GUILayout.Height(_scrollViewHeight));
                            GUI.skin.button.alignment = TextAnchor.MiddleLeft;

                            int _labelWidth_index = _networkedViewsKey.Length.ToString().Length * 12;
                            for (int i = 0; i < _networkedViewsKey.Length; i++)
                            {
                                int _key = _networkedViewsKey[i];

                                if (FMWebSocket.NetworkViews.ContainsKey(_key))
                                {
                                    if (FMWebSocket.NetworkViews.TryGetValue(_key, out FMWebSocketView _view))
                                    {
                                        if (_view == null)
                                        {
                                            FMWebSocket.Action_EditorUnregisterRoomNetworkedView(_key);
                                            FMWebSocket.UpdateRoomViewIDsAll();
                                        }
                                        else
                                        {
                                            if (_key != _view.GetViewID()) _view.SetViewID(_key);
                                            EditorGUILayout.BeginHorizontal();
                                            GUILayout.Label(@$"{_key}", GUILayout.Width(_labelWidth_index));
                                            GUILayout.Label(@$"View {_view.GetViewID()}", GUILayout.Width(60));
                                            EditorGUILayout.ObjectField(_view, typeof(FMWebSocketView), true, GUILayout.Width(EditorGUIUtility.currentViewWidth - 160 - _labelWidth_index));
                                            GUILayout.EndHorizontal();
                                        }
                                    }
                                }
