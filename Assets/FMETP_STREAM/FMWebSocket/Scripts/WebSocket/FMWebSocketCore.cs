    ...
    public class FMWebSocketCore : MonoBehaviour
    {
        ...
        internal FMWebSocketConnectionStatus connectionStatus
        {
            ...
            set
            {
                ...
                Manager.OnConnectionStatusUpdated(_connectionStatus);
            }
        }
        ...
        internal bool wsJoinedRoom { get { return Manager.Settings.wsJoinedRoom; } set { Manager.OnJoinedRoomUpdated(value); } }
        internal bool wsRoomMaster { get { return Manager.Settings.wsRoomMaster; } set { Manager.OnRoomMasterStatusUpdated(value); } }
        ...
        internal void OnMessageCheck(string _msg)
        {
            ...
            if (_msg.Contains("fmevent:"))
            {
                ...
                if (decodedResult.Length == 3)
                {
                    ...
                    // DebugLog("decodedType: " + decodedType);
                    // DebugLog("decodedValue: " + decodedValue);
                    ...
                    switch (decodedType)
                    {
                        ...
                        case "OnRoomClientsUpdated":
                            ...
                            break;
                        case "OnRoomMasterRequestEvent":
                            if (decodedValue.ToLower().Contains("registered")) Manager.OnRoomMasterRequestEvent(true);
                            break;
                        ...
                        case "ClientImageUpload":
                            print(_msg);
                            break;
                    }

                }
                else if (_msg.Contains("ClientImageUpload"))
                {
                    // string tempString = _msg.Remove("fmevent:");
                    ImageDownloader.Instance.SetupImage(_msg);
                    // print(_msg);                }
                }
                else if (_msg.Contains("webPoints"))
                {
                    ImageDownloader.Instance.MovePointer(_msg);
                }
        ...
        internal void SendNetworkInfo(byte[] _inputBytes, FMWebSocketSendType _inputSendType) { SendNetworkInfo(_inputBytes, _inputSendType, null); }
        internal void SendNetworkInfo(byte[] _inputBytes, FMWebSocketSendType _inputSendType, string _targetID) { Send(_inputBytes, _inputSendType, _targetID, 10); }

        public void Send(byte[] _byteData, FMWebSocketSendType _type, string _targetID, int _dataFormat = 0)
        {
            if (NetworkType == FMWebSocketNetworkType.WebSocket || connectionStatus != FMWebSocketConnectionStatus.FMWebSocketConnected) return;
            if (!wsJoinedRoom)
            {
                Debug.LogWarning("skip, not joined room yet");
                return;
            }
            //if (PingMS > 200) return;//skip it if ping too high..

            byte[] _meta = new byte[4];
            _meta[0] = (byte)(_dataFormat);//raw byte(0), encoded string(1), networkObject(10)
            switch (_type)
            {
        ...
        public void Send(string _stringData, FMWebSocketSendType _type, string _targetID) { Send(Encoding.ASCII.GetBytes(_stringData), _type, _targetID, 1); }
