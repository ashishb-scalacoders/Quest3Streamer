...
namespace FMSolution.FMETP
{
    ...
    public class WebcamManager : MonoBehaviour
    {
        ...
        private async void RestartWebcamAsync()
        {
            canRestart = false;
            Action_StopWebcam();
            await initAndWaitForWebCamTextureTask();
            canRestart = true;
        }
        ...
        private void CalculateBackgroundQuad()
        {
        ...
        {
            if (!enabled || !gameObject.activeSelf) return;
            if (!isInitWaiting) initAndWaitForWebCamTextureAsync();
        }
        private void OnEnable() { Action_StartWebcam(); }
        private void OnDisable() { Action_StopWebcam(); }

        private async void initAndWaitForWebCamTextureAsync()
        {
        ...
        private async Task initAndWaitForWebCamTextureTask()
        {
            if (isInitWaiting) return;
            isInitWaiting = true;
            ...
            if (textures.Length > 0)
            {
                ...
                int initFrameCount = Time.frameCount;
                try
                {
                    while (!stoppedOrCancelled() && webCamTexture.width <= 16)
                    {
                        if (Time.frameCount - initFrameCount > timeoutFrameCount) break;
                        await FMCoreTools.AsyncTask.Yield();
                        if (webCamTexture.width <= 16) await FMCoreTools.AsyncTask.Delay(1);
                    }
                }
                catch { }
