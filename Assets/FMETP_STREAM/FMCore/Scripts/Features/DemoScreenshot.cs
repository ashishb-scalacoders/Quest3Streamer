...
namespace FMSolution.FMETP
{
    public class DemoScreenshot : MonoBehaviour
    {
        ...
        private IEnumerator SaveScreenshotPano()
        {
            ...
            byte[] bytes = saveFormat == screenshotSaveFormat.jpg ? pano.FMEncodeToJPG(quality) : pano.EncodeToPNG();

            string path = Directory.GetParent(Application.dataPath).ToString() + "/Screenshots/";
            if (!Directory.Exists(path)) Directory.CreateDirectory(path);
            string SavePath = path + SceneManager.GetActiveScene().name + order + "." + saveFormat.ToString();
