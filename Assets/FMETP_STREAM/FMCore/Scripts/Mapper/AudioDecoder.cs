...
namespace FMSolution.FMETP
{
    ...
    public class AudioDecoder : MonoBehaviour
    {
        ...
        private void CreateClipWeb(byte[] inputByteData)
        {
            ...
            initialiseAudioContextJavascript(label);
            if (SourceSampleRate != 0 && SourceSampleRate != 0)
            {
                StreamAudio((int)label, SourceChannels, inputByteData.Length / 2, SourceSampleRate, inputByteData, inputByteData.Length);
            }
        }

        [DllImport("__Internal")] private static extern void StreamAudio(int NUM_LABEL, int NUM_CHANNELS, int NUM_SAMPLES, int SAMPLE_RATE, byte[] PCM16_BYTES, int PCM16_BYTES_SIZE);
        ...
        private static void initialiseAudioContextJavascript(UInt16 inputLabel)
        {
            ...
            if (!string.IsNullOrEmpty(initialisedJavascript))
            {
                if (!initialisedJavascript.Contains($@"startTimeArray[{inputLabel}] = 0;"))
                {
                    string javascript = ""
                        + $@"startTimeArray[{inputLabel}] = 0;" + "\n"
                        + $@"audioCtxArray[{inputLabel}] = new AudioContext();" + "\n"
                        ;
